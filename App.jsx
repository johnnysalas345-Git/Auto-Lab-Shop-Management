      ...
      <div style={{ ...styles.app, background: COLORS.bg }}>
        ...
        {showWorkOrdersView && (
          <div style={{ position: 'fixed', inset: 0, background: COLORS.bg, zIndex: 1000, display: 'flex', flexDirection: 'column' }}>
            <div style={{ ...styles.header, background: COLORS.primaryLight, borderColor: COLORS.border, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ color: COLORS.text, margin: 0, fontSize: 24, fontWeight: '900' }}>ALL WORK ORDERS</h2>
            ...
            <div style={{ flex: 1, padding: '24px', overflowY: 'auto' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: 20 }}>
                {workOrders.length === 0 ? (
                  <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: 60, color: COLORS.textLight }}>
                    <div style={{ fontSize: 18, fontWeight: '700' }}>No work orders found</div>
                  </div>
                ) : (
                  workOrders.map(wo => {
                    const woData = wo.data || wo;
                    ...
                    const veh = vehicles.find(v => v.id === woData.vehicle_id);
                    return (
                      <div key={wo.id} style={{ ...styles.woCard, background: COLORS.cardBg, borderColor: COLORS.border, border: '2px solid', borderRadius: 12, padding: 20, boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
                        <div style={{ fontSize: 18, fontWeight: '900', color: COLORS.primary, marginBottom: 12 }}>WO #{wo.id ? wo.id.replace('wo', '') : 'N/A'}</div>
                        <div style={{ fontSize: 14, fontWeight: '800', color: COLORS.text, marginBottom: 8 }}>{cust?.data?.name}</div>
                        <div style={{ fontSize: 13, fontWeight: '700', color: COLORS.textLight, marginBottom: 12 }}>
                          {veh?.data?.year} {veh?.data?.make} {veh?.data?.model}
                        </div>
                        <div style={{ fontSize: 12, color: COLORS.text, marginBottom: 12, fontWeight: '700', lineHeight: '1.6' }}>
                          <strong>Complaint:</strong> {woData.complaint}
                        </div>
                        {woData.notes && (
                          <div style={{ fontSize: 12, color: COLORS.textLight, marginBottom: 12, fontWeight: '700' }}>
                            <strong>Notes:</strong> {woData.notes}
                          </div>
                        )}
                        <div style={{...getStatusStyle(woData.status), padding: '6px 12px', borderRadius: 6, display: 'inline-block', fontWeight: '800' }}>
                          {woData.status ? woData.status.replace('_', ' ').toUpperCase() : 'UNKNOWN'}
                        </div>
                        <div style={{ fontSize: 11, color: COLORS.textLight, marginTop: 12, fontWeight: '700' }}>
                          Opened: {woData.opened_at}
                        </div>
                      </div>
                    );
                  })
