...
export default function GarageDashboard() {
  ...
  const [showWorkOrdersView, setShowWorkOrdersView] = useState(false);
  ...
  if (activeTab === 'dashboard') {
    ...
    return (
      <div style={{ ...styles.app, background: COLORS.bg }}>
        ...
        <div style={{ ...styles.actionsBar, background: COLORS.primaryLight }}>
          <div style={styles.actionBarContent}>
            <div style={styles.actionButtonsContainer}>
              ...
              <button 
                onClick={() => setShowWorkOrdersView(true)}
                style={{ ...styles.actionBtn, background: '#F2A900' }}
                onMouseEnter={(e) => { e.currentTarget.style.background = '#FFD84D'; }}
                onMouseLeave={(e) => { e.currentTarget.style.background = '#F2A900'; }}
              >
                <span style={{ fontSize: 18, fontWeight: 'bold' }}>⚙️</span>
                <span>Work Orders & Estimates</span>
              </button>
              <button 
                style={{ ...styles.actionBtn, background: '#F2A900' }}
                onMouseEnter={(e) => { e.currentTarget.style.background = '#FFD84D'; }}
                      ...
                      <div key={wo.id} style={{ ...styles.woCard, background: COLORS.primaryLight, borderColor: COLORS.border }}>
                        <div style={styles.woContent}>
                          ...
                          <div style={{...styles.woStatus, ...getStatusStyle(woData.status)}}>
                            {woData.status ? woData.status.replace('_', ' ').toUpperCase() : 'UNKNOWN'}
                          </div>
                        </div>
                        ▶️
                      </div>
        ...
        {showWorkOrdersView && (
          <div style={{ position: 'fixed', inset: 0, background: COLORS.bg, zIndex: 1000, display: 'flex', flexDirection: 'column' }}>
            <div style={{ ...styles.header, background: COLORS.primaryLight, borderColor: COLORS.border, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 24px' }}>
              <h2 style={{ color: COLORS.text, margin: 0, fontSize: 24, fontWeight: '900' }}>WORK ORDERS & ESTIMATES</h2>
              <button onClick={() => setShowWorkOrdersView(false)} style={{ background: 'none', border: 'none', fontSize: 32, cursor: 'pointer', color: COLORS.text, fontWeight: '900' }}>✕</button>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
              <div style={{ background: COLORS.cardBg, borderRadius: 8, border: '1px solid', borderColor: COLORS.border, overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: COLORS.primaryLight, borderBottom: '2px solid', borderColor: COLORS.border }}>
                      <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: 14, fontWeight: '900', color: COLORS.text, minWidth: 180 }}>Customer Name</th>
                      <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: 14, fontWeight: '900', color: COLORS.text, minWidth: 140 }}>Phone</th>
                      <th style={{ padding: '12px 16px', textAlign: 'center', fontSize: 14, fontWeight: '900', color: COLORS.text, minWidth: 120 }}>Brand</th>
                      <th style={{ padding: '12px 16px', textAlign: 'center', fontSize: 14, fontWeight: '900', color: COLORS.text, minWidth: 140 }}>Model</th>
                      <th style={{ padding: '12px 16px', textAlign: 'center', fontSize: 14, fontWeight: '900', color: COLORS.text, minWidth: 140 }}>License Plate</th>
                      <th style={{ padding: '12px 16px', textAlign: 'center', fontSize: 14, fontWeight: '900', color: COLORS.text, minWidth: 160 }}>WO/Est Number</th>
                      <th style={{ padding: '12px 16px', textAlign: 'center', fontSize: 14, fontWeight: '900', color: COLORS.text, minWidth: 140 }}>Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {workOrders.length === 0 ? (
                      <tr>
                        <td colSpan="7" style={{ padding: 40, textAlign: 'center', color: COLORS.textLight }}>No work orders or estimates found</td>
                      </tr>
                    ) : (
                      workOrders.map(wo => {
                        const woData = wo.data || wo;
                        ...
                        return (
                          ...
                          >
                            <td style={{ padding: '12px 16px', fontSize: 15, color: COLORS.text, fontWeight: '800' }}>{cust?.data?.name}</td>
                            <td style={{ padding: '12px 16px', fontSize: 15, color: COLORS.text, fontWeight: '800' }}>{cust?.data?.phone}</td>
                            <td style={{ padding: '12px 16px', fontSize: 15, color: COLORS.text, fontWeight: '800', textAlign: 'center' }}>{veh?.data?.make}</td>
                            <td style={{ padding: '12px 16px', fontSize: 15, color: COLORS.text, fontWeight: '800', textAlign: 'center' }}>{veh?.data?.model}</td>
                            <td style={{ padding: '12px 16px', fontSize: 15, color: COLORS.primary, fontWeight: '900', textAlign: 'center' }}>{veh?.data?.license_plate}</td>
                            <td style={{ padding: '12px 16px', fontSize: 15, color: COLORS.primary, fontWeight: '900', textAlign: 'center' }}>
                              {woData.type === 'estimate' ? 'EST' : woData.type === 'inspection' ? 'INSP' : 'WO'} #{wo.id ? wo.id.replace(/^(wo|est|insp)/, '') : 'N/A'}
                            </td>
                            <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                              <div style={{...getStatusStyle(woData.status), padding: '4px 8px', borderRadius: 4, display: 'inline-block', fontWeight: '800', fontSize: 12 }}>
                                {woData.status ? woData.status.replace('_', ' ').toUpperCase() : 'UNKNOWN'}
                              </div>
                            </td>
                          </tr>
        ...
        {selectedWorkOrder && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1001 }}>
            <div style={{ background: COLORS.cardBg, borderRadius: 8, padding: 32, maxWidth: 700, width: '90%', maxHeight: '90vh', overflowY: 'auto' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <h3 style={{ color: COLORS.text, margin: 0, fontSize: 20, fontWeight: '900' }}>
                  {selectedWorkOrder.data?.type === 'estimate' ? 'ESTIMATE' : selectedWorkOrder.data?.type === 'inspection' ? 'INSPECTION' : 'WORK ORDER'} DETAILS
                </h3>
                <button onClick={() => setSelectedWorkOrder(null)} style={{ background: 'none', border: 'none', fontSize: 28, cursor: 'pointer', color: COLORS.text, fontWeight: '900' }}>✕</button>
              </div>
              
              <div style={{ marginBottom: 20, padding: 16, background: COLORS.primaryLight, borderRadius: 8 }}>
                <div style={{ fontSize: 12, fontWeight: '900', color: COLORS.textLight, textTransform: 'uppercase', marginBottom: 8 }}>ID</div>
                <div style={{ fontSize: 18, fontWeight: '900', color: COLORS.primary }}>
                  {selectedWorkOrder.data?.type === 'estimate' ? 'EST' : selectedWorkOrder.data?.type === 'inspection' ? 'INSP' : 'WO'} #{selectedWorkOrder.id ? selectedWorkOrder.id.replace(/^(wo|est|insp)/, '') : 'N/A'}
                </div>
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
                <div>
                  <div style={{ fontSize: 12, fontWeight: '900', color: COLORS.textLight, textTransform: 'uppercase', marginBottom: 4 }}>Customer</div>
                  <div style={{ fontSize: 16, fontWeight: '900', color: COLORS.text }}>
                    {customers.find(c => c.id === selectedWorkOrder.data?.customer_id)?.data?.name}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: 12, fontWeight: '900', color: COLORS.textLight, textTransform: 'uppercase', marginBottom: 4 }}>Phone</div>
                  <div style={{ fontSize: 16, fontWeight: '900', color: COLORS.text }}>
                    {customers.find(c => c.id === selectedWorkOrder.data?.customer_id)?.data?.phone}
                  </div>
                </div>
              </div>

              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 12, fontWeight: '900', color: COLORS.textLight, textTransform: 'uppercase', marginBottom: 4 }}>Vehicle</div>
                <div style={{ fontSize: 16, fontWeight: '900', color: COLORS.text }}>
                  {vehicles.find(v => v.id === selectedWorkOrder.data?.vehicle_id)?.data?.year} {vehicles.find(v => v.id === selectedWorkOrder.data?.vehicle_id)?.data?.make} {vehicles.find(v => v.id === selectedWorkOrder.data?.vehicle_id)?.data?.model} ({vehicles.find(v => v.id === selectedWorkOrder.data?.vehicle_id)?.data?.license_plate})
                </div>
              </div>

              {selectedWorkOrder.data?.type !== 'estimate' && selectedWorkOrder.data?.type !== 'inspection' && (
                <>
                  <div style={{ marginBottom: 20 }}>
                    <div style={{ fontSize: 12, fontWeight: '900', color: COLORS.textLight, textTransform: 'uppercase', marginBottom: 4 }}>Complaint</div>
                    <div style={{ fontSize: 14, fontWeight: '800', color: COLORS.text, lineHeight: '1.6' }}>
                      {selectedWorkOrder.data?.complaint}
                    </div>
                  </div>

                  {selectedWorkOrder.data?.notes && (
                    <div style={{ marginBottom: 20 }}>
                      <div style={{ fontSize: 12, fontWeight: '900', color: COLORS.textLight, textTransform: 'uppercase', marginBottom: 4 }}>Notes</div>
                      <div style={{ fontSize: 14, fontWeight: '800', color: COLORS.text, lineHeight: '1.6' }}>
                        {selectedWorkOrder.data?.notes}
                      </div>
                    </div>
              ...
              {selectedWorkOrder.data?.type === 'estimate' && (
                <>
                  <div style={{ marginBottom: 20 }}>
                    <div style={{ fontSize: 12, fontWeight: '900', color: COLORS.textLight, textTransform: 'uppercase', marginBottom: 4 }}>Description</div>
                    <div style={{ fontSize: 14, fontWeight: '800', color: COLORS.text, lineHeight: '1.6' }}>
                      {selectedWorkOrder.data?.description}
                    </div>
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
                    <div>
                      <div style={{ fontSize: 12, fontWeight: '900', color: COLORS.textLight, textTransform: 'uppercase', marginBottom: 4 }}>Parts Cost</div>
                      <div style={{ fontSize: 16, fontWeight: '900', color: COLORS.primary }}>CI${selectedWorkOrder.data?.parts_cost || '0.00'}</div>
                    </div>
                    <div>
                      <div style={{ fontSize: 12, fontWeight: '900', color: COLORS.textLight, textTransform: 'uppercase', marginBottom: 4 }}>Labor Cost</div>
                      <div style={{ fontSize: 16, fontWeight: '900', color: COLORS.primary }}>CI${selectedWorkOrder.data?.labor_cost || '0.00'}</div>
                    </div>
                  </div>
                </>
              )}

              {selectedWorkOrder.data?.type === 'inspection' && (
                <>
                  <div style={{ marginBottom: 20 }}>
                    <div style={{ fontSize: 12, fontWeight: '900', color: COLORS.textLight, textTransform: 'uppercase', marginBottom: 4 }}>Findings</div>
                    <div style={{ fontSize: 14, fontWeight: '800', color: COLORS.text, lineHeight: '1.6' }}>
                      {selectedWorkOrder.data?.findings}
                    </div>
                  </div>

                  {selectedWorkOrder.data?.recommendations && (
                    <div style={{ marginBottom: 20 }}>
                      <div style={{ fontSize: 12, fontWeight: '900', color: COLORS.textLight, textTransform: 'uppercase', marginBottom: 4 }}>Recommendations</div>
                      <div style={{ fontSize: 14, fontWeight: '800', color: COLORS.text, lineHeight: '1.6' }}>
                        {selectedWorkOrder.data?.recommendations}
                      </div>
                    </div>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 12, fontWeight: '900', color: COLORS.textLight, textTransform: 'uppercase', marginBottom: 4 }}>Status</div>
                <div style={{...getStatusStyle(selectedWorkOrder.data?.status), padding: '8px 12px', borderRadius: 6, display: 'inline-block', fontWeight: '900', fontSize: 14 }}>
                  {selectedWorkOrder.data?.status ? selectedWorkOrder.data?.status.replace('_', ' ').toUpperCase() : 'UNKNOWN'}
                </div>
              </div>

              <button onClick={() => setSelectedWorkOrder(null)} style={{ ...styles.actionBtn, background: '#4CAF50', border: '1px solid #4CAF50', width: '100%', justifyContent: 'center' }}>
                Close
              </button>
            </div>
          </div>
        ...
        <div style={{ ...styles.section, background: COLORS.cardBg, borderColor: COLORS.border, border: '1px solid', borderRadius: 8, overflow: 'hidden' }}>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: COLORS.primaryLight, borderBottom: '2px solid', borderColor: COLORS.border }}>
                  <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: 14, fontWeight: '900', color: COLORS.text, minWidth: 200 }}>Name</th>
                  <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: 14, fontWeight: '900', color: COLORS.text, minWidth: 140 }}>Phone</th>
                  <th style={{ padding: '12px 16px', textAlign: 'center', fontSize: 14, fontWeight: '900', color: COLORS.text, minWidth: 120 }}>Brand</th>
                  <th style={{ padding: '12px 16px', textAlign: 'center', fontSize: 14, fontWeight: '900', color: COLORS.text, minWidth: 140 }}>Model</th>
                  <th style={{ padding: '12px 16px', textAlign: 'center', fontSize: 14, fontWeight: '900', color: COLORS.text, minWidth: 140 }}>License Plate</th>
                      ...
                      <React.Fragment key={customer.id}>
                        {custVehicles.length === 0 ? (
                          <tr style={{ borderBottom: '1px solid', borderColor: COLORS.border }}>
                            <td style={{ padding: '12px 16px', fontSize: 15, color: COLORS.text, fontWeight: '900', cursor: 'pointer' }} onClick={() => setEditingCustomer(customer)} title="Click to edit">{customer.data?.name}</td>
                            <td style={{ padding: '12px 16px', fontSize: 15, color: COLORS.text, fontWeight: '900', cursor: 'pointer' }} onClick={() => setEditingCustomer(customer)} title="Click to edit">{customer.data?.phone}</td>
                          ...
                          custVehicles.map((vehicle, idx) => (
                            <tr key={vehicle.id} style={{ borderBottom: '1px solid', borderColor: COLORS.border }}>
                              <td style={{ padding: '12px 16px', fontSize: 15, color: COLORS.text, fontWeight: '900', cursor: 'pointer' }} onClick={() => setEditingCustomer(customer)} title="Click to edit">{idx === 0 ? customer.data?.name : ''}</td>
                              <td style={{ padding: '12px 16px', fontSize: 15, color: COLORS.text, fontWeight: '900', cursor: 'pointer' }} onClick={() => setEditingCustomer(customer)} title="Click to edit">{idx === 0 ? customer.data?.phone : ''}</td>
                              <td style={{ padding: '12px 16px', fontSize: 15, color: COLORS.text, fontWeight: '900', textAlign: 'center', cursor: 'pointer' }} onClick={() => setEditingVehicle(vehicle)} title="Click to edit">{vehicle.data?.make}</td>
                              <td style={{ padding: '12px 16px', fontSize: 15, color: COLORS.text, fontWeight: '900', textAlign: 'center', cursor: 'pointer' }} onClick={() => setEditingVehicle(vehicle)} title="Click to edit">{vehicle.data?.model}</td>
                              <td style={{ padding: '12px 16px', fontSize: 15, color: COLORS.primary, fontWeight: '900', textAlign: 'center', cursor: 'pointer' }} onClick={() => setEditingVehicle(vehicle)} title="Click to edit">{vehicle.data?.license_plate}</td>
                              <td style={{ padding: '12px 16px', textAlign: 'center' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: COLORS.cardBg, borderRadius: 8, padding: 24, maxWidth: 500, width: '90%', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <button onClick={() => setEditingCustomer(null)} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: COLORS.text }}>✕</button>
            </div>
            <EditCustomerForm customer={editingCustomer} onSave={() => { loadData(); setEditingCustomer(null); }} onCancel={() => setEditingCustomer(null)} />
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: COLORS.cardBg, borderRadius: 8, padding: 24, maxWidth: 500, width: '90%', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <button onClick={() => setEditingVehicle(null)} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: COLORS.text }}>✕</button>
            </div>
            <EditVehicleForm vehicle={editingVehicle} onSave={() => { loadData(); setEditingVehicle(null); }} onCancel={() => setEditingVehicle(null)} />
          </div>
        </div>
      )}
    </div>
...
function EditCustomerForm({ customer, onSave, onCancel }) {
  const [formData, setFormData] = useState({ name: customer.data?.name || '', phone: customer.data?.phone || '', email: customer.data?.email || '', address: customer.data?.address || '' });
  const [loading, setLoading] = useState(false);
...
function EditVehicleForm({ vehicle, onSave, onCancel }) {
  const [formData, setFormData] = useState({ year: vehicle.data?.year || new Date().getFullYear(), make: vehicle.data?.make || '', model: vehicle.data?.model || '', license_plate: vehicle.data?.license_plate || '', vin: vehicle.data?.vin || '' });
  const [loading, setLoading] = useState(false);
...
const styles = {
  ...
  menuLabel: { fontSize: 15, fontWeight: '900', color: '#666666' },
  ...
  menuItemStyle: { display: 'block', width: '100%', padding: '12px 16px', border: 'none', cursor: 'pointer', fontSize: 14, fontWeight: '800', textAlign: 'left', color: '#333333', transition: 'background 0.15s' },
  ...
  avatar: { width: 36, height: 36, borderRadius: '50%', background: COLORS.primary, color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '900', fontSize: 14 },
  ...
  actionBtn: { display: 'flex', alignItems: 'center', gap: 8, color: '#000000', border: '1px solid #000000', borderRadius: 6, padding: '10px 18px', fontWeight: '900', fontSize: 15, cursor: 'pointer' },
  ...
  cashLabel: { fontSize: 12, fontWeight: '900', textTransform: 'uppercase', marginBottom: 2, color: '#333333' },
  cashAmount: { fontSize: 18, fontWeight: '900', color: '#000000' },
  ...
  calendarBtn: { borderRadius: 6, padding: '8px 16px', cursor: 'pointer', fontSize: 16, fontWeight: '900', border: 'none', position: 'absolute', right: 24 },
  filterTabs: { display: 'flex', gap: 6, padding: '12px 24px', borderBottom: '1px solid', flexWrap: 'wrap', alignItems: 'center' },
  filterTab: { padding: '8px 12px', border: '1px solid', borderRadius: 6, cursor: 'pointer', fontSize: 13, fontWeight: '800', whiteSpace: 'nowrap' },
  ...
  techName: { fontSize: 16, fontWeight: '900' },
  jobCount: { fontSize: 14, fontWeight: '800' },
  ...
  jobWONum: { fontSize: 14, fontWeight: '900' },
  jobVehicle: { fontSize: 13, marginTop: 4, fontWeight: '800' },
  ...
  woContent: { flex: 1 },
  woNumber: { fontSize: 15, fontWeight: '900' },
  woCustomer: { fontSize: 14, fontWeight: '800', marginTop: 4 },
  ...
  formInput: { width: '100%', padding: '10px 12px', marginBottom: 10, border: '1px solid', borderColor: COLORS.border, borderRadius: 4, fontSize: 14, fontFamily: 'inherit', fontWeight: '700', boxSizing: 'border-box' },
};
